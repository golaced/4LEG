...
in->sys.min_range=0.707*leg[1].sys.l3*0.44/2;//7.8 4.742556
...
{ u8 i=0,j=0;	
  ...
  //»úÌåÖÐÐÄ
  brain.global.end_pos_global[0].x=brain.sys.leg_local[1].x-leg[4].pos_now[2].x+0.5*brain.now_acc[0]*dt*dt+off_cor[Xr];
	brain.global.end_pos_global[0].y=brain.sys.leg_local[1].y-leg[4].pos_now[2].y+0.5*brain.now_acc[1]*dt*dt+off_cor[Yr];
...
{ static u8 reg_flag;
  u8 i;
	...
	#else
	if(brain.force_stop)
		spd_use=LIMIT(brain.spd,0,1)*k_spd*brain.global.center_stable_weight/1.618;
	else
		spd_use=brain.spd*k_spd*brain.global.center_stable_weight;
	#endif
       ...
       //µãÊ¸Á¿ÓëÖ±Ïß½»µã			
	  ...
	  else//Ç°·½Á½ÌõÍÈ
		{
		  ...
		  f_leg_x1=brain.global.end_pos_global[id_f[1]].x;
			...
			u8 id3;				 
			for(i=1;i<5;i++)
		  if(leg[i].leg_ground&&i!=id_f[0]&&i!=id_f[1])		 
      {
			id3=i;
      break;				
			}
...
{ u8 i,j;
	...
	{fp_point_id[0]=1;fp_point_id[1]=2;   fp_point_id[2]=3;fp_point_id[3]=4;way=2;}
	else if(yaw_in<-yaw_temp&&yaw_in>-90-yaw_temp)//l
	{fp_point_id[0]=4;fp_point_id[1]=3;   fp_point_id[2]=2;fp_point_id[3]=1;way=4;}
	else if((yaw_in<yaw_temp&&yaw_in>=0)||(yaw_in>-yaw_temp&&yaw_in<0))//f
	{fp_point_id[0]=3;fp_point_id[1]=1;   fp_point_id[2]=4;fp_point_id[3]=2;way=1;}
	else//b
	{fp_point_id[0]=2;fp_point_id[1]=4;   fp_point_id[2]=1;fp_point_id[3]=3;way=3;}
...
//		if(i==fp_point_id[2]||i==fp_point_id[3])//ºó
//			switch(way){
//				case 1:limit_x=brain.sys.min_range;limit_y=brain.sys.max_range;break;
//				case 2:limit_x=brain.sys.max_range;limit_y=brain.sys.min_range;break;
//				case 3:limit_x=brain.sys.min_range;limit_y=brain.sys.max_range;break;
//				case 4:limit_x=brain.sys.max_range;limit_y=brain.sys.min_range;break;
//			}
//		else//Ç°
//				limit_x=brain.sys.min_range;limit_y=brain.sys.min_range;
//			
//   limit_x=in->sys.leg_move_range[Xr];
//	 limit_y=in->sys.leg_move_range[Yr];
		
	 //range_out[i]=in_circle(circle_center[0],circle_center[1],limit_x,limit_y,tar_x,tar_y);
	 range_out[i]=check_in_move_range(i,tar_x,tar_y,circle_center[0],circle_center[1],in->sys.min_range,in->sys.max_range);
	 if(brain.rst_all_soft)
	  brain.tabu=need_move_leg=1;  	 
 
	 if(dis[1]*FLAG_IN(spd)<dis[0]*FLAG_IN(spd)&&range_out[i]==0&&fabs(spd)>0)
	 { 
	 leg[i].need_move=3;
	 out_range_move[i]++;
   //need_move_leg=1;	
 ...
 if(brain.ground_leg_num>3&&brain.center_stable)
 {
   ...
   line_function_from_two_point(brain.global.end_pos_global[1].x ,brain.global.end_pos_global[1].y,
	   brain.global.end_pos_global[4].x,brain.global.end_pos_global[4].y,&k1,&b1);
	 
   line_function_from_two_point(brain.global.end_pos_global[2].x ,brain.global.end_pos_global[2].y,
	   brain.global.end_pos_global[3].x,brain.global.end_pos_global[3].y,&k2,&b2);
	 
   cross_point_of_lines( k1,b1, k2, b2,&cro_x,&cro_y);
 
   if(check_point_front_arrow(brain.global.end_pos_global[0].x,brain.global.end_pos_global[0].y,cro_x,cro_y,brain.spd_yaw)==1)
	 {
	   if((cal_dis_of_points(brain.global.end_pos_global[0].x,brain.global.end_pos_global[0].y,cro_x,cro_y)>brain.min_st[1]*0)||
...
//---------------------------------------¹æ»®ÍÈ------------------------------------------------	-
 ...
 if(brain.rst_all_soft){	 
   ...
   for(i=2;i<5;i++){
	 ...
	 id_need_to_move=LIMIT(id_need_to_move,0,4);
	 if(leg[id_need_to_move].need_move==3&&out_range_move[id_need_to_move]>0&&id_need_to_move>0)
	 { out_range_move[id_need_to_move]=0;goto endl;}
 ...
 #if !DENG_LIMIT_TEST	
 leg[in->move_id].leg_ground_fake=0;
   ...
   line_function_from_two_point(brain.global.end_pos_global[fp_point_id[0]].x,brain.global.end_pos_global[fp_point_id[0]].y,
	 ...
	 u8 temp_id;
		  if(can_leg_id[0]==last_move_id){
				if(can_leg_id[1]==last_last_move_id)
				{
				  for(i=1;i<5;i++){
					if(yaw_in>yaw_temp&&yaw_in<90+yaw_temp)
				...
				if(can_leg_id[0]==last_last_move_id)
				{
				  for(i=1;i<5;i++){
					if(yaw_in>yaw_temp&&yaw_in<90+yaw_temp)
					temp_id=trig_list_r[i];
...
tar_x=LIMIT(sin(tar_yaw/ 57.3f)*LIMIT(spd*off_k*in->sys.k_spd_to_range,-20,20),-in->sys.leg_move_range[Xr],in->sys.leg_move_range[Xr]);
tar_y=LIMIT(cos(tar_yaw/ 57.3f)*LIMIT(spd*off_k*in->sys.k_spd_to_range,-20,20),-in->sys.leg_move_range[Yr],in->sys.leg_move_range[Yr]);
leg->pos_tar_trig[2].x=leg->sys.init_end_pos.x*2+tar_x+RANDOM+off_x*cos(tar_yaw/ 57.3f);
leg->pos_tar_trig[2].y=leg->sys.init_end_pos.y*2+tar_y+RANDOM+off_y*sin(tar_yaw/ 57.3f);
...
{
brain.rst_all_soft++;
leg->pos_tar_trig[2].x=leg->sys.init_end_pos.x*2+RANDOM;//+off_x*cos(tar_yaw/ 57.3f);
leg->pos_tar_trig[2].y=leg->sys.init_end_pos.y*2+RANDOM;//+off_y*sin(tar_yaw/ 57.3f);
